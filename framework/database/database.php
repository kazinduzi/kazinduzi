...
class Database extends DbActiveRecord 
{
    ...
    {
        $dbDriverClassName = $this->getConfigValue('driver') ? 'database_driver_' . $this->getConfigValue('driver') : 'database_driver_mysql';
        require_once KAZINDUZI_PATH . '/' . str_replace('_', '/', $dbDriverClassName) . '.php';
        $className = ucfirst(substr($dbDriverClassName, 9));
	$this->driver = new $className($this->getConfigValue(null));
