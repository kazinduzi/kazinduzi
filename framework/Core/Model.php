...
abstract class Model implements IteratorAggregate
{
     ...
     */
    public static function getInstance($className = null, $options = [])
    {
        if (empty($className)) {
     ...
     * @return type
     */
    public function findByPri($pri, $condition = null, $params = [])
    {
        self::$db->clear()
        ->select('*')
        ->from($this->getTable())
     ...
     * @return \Model
     */
    public function findOne($condition = '', $params = [])
    {
        self::$db->clear();
        self::$db->select('*')
        ->from($this->getTable())
     ...
     * @param type $params
     */
    public function findByAttr($attrs, $conds = null, $params = [])
    {
        self::$db->reset();
        if (is_string($attrs)) {
            $attrs = (array) $attrs;
     ...
     * @return class
     */
    public static function find($table, array $args = [])
    {
        $table = strtolower($table);
        $query = 'SELECT * FROM `'.$table.'`';
        $query .= !empty($args['WHERE']) ? ' WHERE '.$args['WHERE'] : '';
    ...
    public function clear_model()
    {
        ...
        $this->values = $this->_changed = $this->relationships = [];
    ...
    public function has($alias, $far_keys)
    {
        ...
        ->where(['AND', $this->hasMany[$alias]['foreign_key'].'='.$this->pk(), ['IN', $this->hasMany[$alias]['far_key'], $far_keys]])
        ->buildQuery();
    ...
    public function reload_columns($force = false)
    {
        if ($force === true or empty($this->tableColumns)) {
            if (isset(self::$_column_cache[$this->modelName])) {
                $this->tableColumns = self::$_column_cache[$this->modelName];
            } else {
                $this->tableColumns = $this->list_columns(true);
                self::$_column_cache[$this->modelName] = $this->tableColumns;
            }
        }
    ...
    private function _related_models($alias)
    {
        if (isset($this->relationships[$alias])) {
            ...
            return $this->relationships[$alias] = self::forgery($this->hasOne[$alias]['model']);
        } elseif (isset($this->belongsTo[$alias])) {
            return $this->relationships[$alias] = self::forgery($this->belongsTo[$alias]['model']);
        } else {
            return false;
        }
    }
